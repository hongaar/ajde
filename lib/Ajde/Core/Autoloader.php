...
class Ajde_Core_Autoloader
{
    ...
    public static $incompatibleClasses = [
    ...
    public static function register($dirPrepend = null)
    {
        ...
        set_include_path(get_include_path() . PATH_SEPARATOR . $_SERVER['DOCUMENT_ROOT'] . str_replace('/index.php', '',
                $_SERVER['PHP_SELF']) . DIRECTORY_SEPARATOR . LIB_DIR);

        // Get namespaces from Config
        $defaultNamespaces = ['Ajde', 'Zend', 'HTMLPurifier'];
        ...
        spl_autoload_register(["Ajde_Core_Autoloader", "autoload"]);
    ...
    public static function autoload($className)
    {
        ...
        if ($isNamespace) {
            ...
            if (substr_count($className, 'Controller') > 0) {
                ...
                if (strtolower(substr($controllerName, 1)) != substr($controllerName,
                        1)
                ) { // See if we got more capitals
                    // ModuleSubcontrollerController.php naming
                    $combinedName = substr($controllerName, 0, 1) . preg_replace('/([A-Z])/', ':\1',
                            substr($controllerName, 1));
