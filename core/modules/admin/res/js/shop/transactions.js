;
if (typeof App === "undefined") {
    App = function () {
    }
}
if (typeof App.Admin === "undefined") {
    App.Admin = function () {
    }
}
if (typeof App.Admin.Shop === "undefined") {
    App.Admin.Shop = function () {
    }
}
App.Admin.Shop.Transactions = function() {

    var infoHandler		= AC.Core.Alert.show;
    var warningHandler	= AC.Core.Alert.warning;
    var errorHandler	= AC.Core.Alert.error;

    var btnMarkPaidHandler = function(e) {

        id = false;
        var form = $(this).parents('form');
        var self = this;
        var data = form.serializeArray();
        var value = $(this).data('id');

        var count = 0;
        for (elm in data) {
            if (data[elm].name == 'id[]') {
                if (id !== false && data[elm].value != id) {
                    delete data[elm];
                } else {
                    count++;
                }
            }
        }

        if (id !== false && count == 0) {
            data.push({
                name: 'id[]',
                value: id
            });
            count = 1;
        }

        if (count > 0) {
            var url = 'admin/shop:markPaid.json';
            $.post(url, data, function(data) {
                if (data.success === true) {
                    infoHandler(data.message);
                    if (AC && AC.Crud && AC.Crud.List) {
                        AC.Crud.List.updateView($('form.ACCrudList').children(':eq(0)'));
                    }
                } else {
                    warningHandler(data.message);
                }
            }, 'json').error(function(jqXHR, message, exception) {
                if (exception == 'Unauthorized' || exception == 'Forbidden') {
                    warningHandler(i18n.forbiddenWarning);
                } else {
                    errorHandler(i18n.requestError + ' (' + exception + ')');
                }
            });
        } else {
            infoHandler('Nothing selected');
        }
    };

    var btnLinkHandler = function(e) {
        var secret = $(this).closest('tr').find('span[data-secret]').data('secret');
        e.stopPropagation();
        e.preventDefault();
        window.open('shop/transaction:view/' + secret + '.html');
    };

    return {

        init: function() {
            $('form.ACCrudList thead a.btn.paid').live('click', btnMarkPaidHandler);
            $('form.ACCrudList td.buttons a.link').live('click', btnLinkHandler);
        }

    };
}();

$(document).ready(function() {
    App.Admin.Shop.Transactions.init();
});
